defmodule JumubaseWeb.Internal.PerformanceControllerTest do
  ...
  describe "schedule/2" do
    for role <- roles_except("local-organizer") do
      @tag login_as: role
      test "lets #{role} users schedule a contest's performances", %{conn: conn, contest: c} do
        conn = get(conn, Routes.internal_contest_performance_path(conn, :schedule, c))
    ...
    test "lets local organizers schedule an own contest's performances", %{conn: conn, user: u} do
      own_c = insert_own_contest(u)
      conn = get(conn, Routes.internal_contest_performance_path(conn, :schedule, own_c))
      assert html_response(conn, 200) =~ "Schedule performances"
    end

    @tag login_as: "local-organizer"
    test "redirects local organizers when trying to schedule a foreign contest's performances", %{conn: conn, contest: c} do
      conn = get(conn, Routes.internal_contest_performance_path(conn, :schedule, c))
      assert_unauthorized_user(conn)
    end

    test "redirects guests when trying to schedule a contest's performances", %{conn: conn, contest: c} do
      conn = get(conn, Routes.internal_contest_performance_path(conn, :schedule, c))
      assert_unauthorized_guest(conn)
    end
  end
