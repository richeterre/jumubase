defmodule JumubaseWeb.PageControllerTest do
  ...
  describe "edit_registration/2" do
    ...
    test "lets users access a registration with a valid edit code within the deadline", %{
      conn: conn,
      contest: c,
      performance: p
    } do
      conn =
        post(conn, Routes.page_path(conn, :lookup_registration), search: %{edit_code: p.edit_code})

      assert redirected_to(conn) ==
               Routes.performance_path(conn, :edit, c, p, edit_code: p.edit_code)
    end
    ...
    test "shows an error when submitting an unknown edit code", %{conn: conn} do
      conn =
        post(conn, Routes.page_path(conn, :lookup_registration), search: %{edit_code: "unknown"})
